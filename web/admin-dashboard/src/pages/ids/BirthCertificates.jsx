...
import { Search, FileText, Eye, Download, Printer } from 'lucide-react';
...
export default function BirthCertificates() {
    ...
    const handlePreview = async (citizen) => {
        const url = `http://localhost:8001/api/v1/documents/preview/?type=birth_certificate&citizen_id=${citizen.national_id}`;
        setPreviewUrl(url);
        setSelectedCitizen(citizen);
    };

    const handleDownload = (citizen) => {
        const url = `http://localhost:8001/api/v1/documents/download/?type=birth_certificate&citizen_id=${citizen.national_id}`;
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 min-h-screen pb-10">
            ...
            <div className="flex justify-between items-end border-b border-[var(--color-BORDER)] pb-6">
                <div>
                    <h2 className="text-2xl font-bold text-[var(--color-text-MAIN)] uppercase tracking-tight flex items-center gap-3">
                        <FileText className="text-[var(--color-ACCENT)]" /> Birth Certificates
                    </h2>
                    <p className="text-[var(--color-text-MUTED)] mt-1 text-sm">Civil registration documents.</p>
                </div>
            </div>
            ...
            <div className="bg-[var(--color-bg-MAIN)] border border-[var(--color-BORDER)] shadow-sm overflow-hidden min-h-[400px]">
                <table className="w-full text-left text-sm">
                    <thead className="bg-[var(--color-bg-SURFACE)] border-b border-[var(--color-BORDER)]">
                        <tr>
                            <th className="px-6 py-3 font-ocr text-xs text-[var(--color-text-MUTED)] uppercase tracking-wider">Child Name</th>
                            <th className="px-6 py-3 font-ocr text-xs text-[var(--color-text-MUTED)] uppercase tracking-wider">Date of Birth</th>
                            <th className="px-6 py-3 font-ocr text-xs text-[var(--color-text-MUTED)] uppercase tracking-wider">Place of Birth</th>
                    ...
                    <tbody className="divide-y divide-[var(--color-BORDER)]">
                        {loading ? (
                            ...
                            <tr><td colSpan="4" className="px-6 py-12 text-center text-[var(--color-text-MUTED)]">No records found.</td></tr>
                        ) : (
                            citizens.map(c => (
            ...
            {previewUrl && (
                <DocumentPreview
                    title={`Birth Certificate: ${selectedCitizen?.national_id}`}
                    pdfUrl={previewUrl}
