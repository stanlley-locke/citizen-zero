...
import { Search, CreditCard, Eye, Download, Printer, ChevronLeft, ChevronRight, X } from 'lucide-react';
import ENDPOINTS from '../../services/apiConfig';
import DocumentPreview from '../../components/documents/DocumentPreview';

// Use exact same debounce logic as CitizenList
...
export default function NationalIDs() {
    ...
    const handlePreview = async (citizen) => {
        ...
        const url = `http://localhost:8001/api/v1/documents/preview/?type=national_id&citizen_id=${citizen.national_id}`;
        setPreviewUrl(url);
        setSelectedCitizen(citizen);
    };

    const handleDownload = (citizen) => {
        const url = `http://localhost:8001/api/v1/documents/download/?type=national_id&citizen_id=${citizen.national_id}`;
        ...
        <div className="space-y-6 animate-in fade-in duration-500 min-h-screen pb-10">
            ...
            <div className="flex justify-between items-end border-b border-[var(--color-BORDER)] pb-6">
                <div>
                    <h2 className="text-2xl font-bold text-[var(--color-text-MAIN)] uppercase tracking-tight flex items-center gap-3">
                        <CreditCard className="text-[var(--color-ACCENT)]" /> National Identity Cards
                    </h2>
                    <p className="text-[var(--color-text-MUTED)] mt-1 text-sm">Issuance and managment of ID-1 credentials.</p>
                </div>
            </div>
            ...
            <div className="flex gap-4 items-center bg-[var(--color-bg-MAIN)] p-4 border border-[var(--color-BORDER)] shadow-sm">
                <div className="relative flex-1">
                    ...
                    <input
                        type="text"
                        placeholder="Search Citizen to Issue/View ID..."
                            ...
                            citizens.map(c => (
                                <tr key={c.national_id} className="hover:bg-[var(--color-bg-SURFACE)] transition-colors">
                                    <td className="px-6 py-4">
                                        ...
                                        <div className="text-xs text-[var(--color-text-MUTED)]">{c.date_of_birth} | {c.county_of_birth}</div>
                                    </td>
                                    <td className="px-6 py-4 font-mono text-[var(--color-text-MAIN)]">{c.national_id}</td>
                                    <td className="px-6 py-4">
            ...
            {previewUrl && (
                <DocumentPreview
                    title={`National ID: ${selectedCitizen?.national_id}`}
                    pdfUrl={previewUrl}
